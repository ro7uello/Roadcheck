...
const mapLayout = [
  ["road62", "road4", "road2", "road2", "road3"],
  ["road62", "road4", "road2", "road2", "road3"],
  ["road63", "road4", "road2", "road2", "road3"],
  ["int12", "int12", "int12", "int12", "int4"],
  ["road48", "road48", "road48", "int14", "int13"],
  ["road48", "road48", "int14", "road48", "int13"],
  ["road48", "int14", "road48", "road48", "int13"],
  ["int9", "road48", "road48", "road48", "int13"],
  ["road48", "int15", "road48", "road48", "int13"],
  ["road48", "road48", "int15", "road48", "int13"],
  ["road48", "road48", "road48", "int15", "int13"],
  ["int10", "int10", "int10", "int10", "int1"],
  ["road61", "road76", "road76", "road76", "road76"],
  ["road62", "road4", "road2", "road2", "road3"],
  ["road62", "road4", "road2", "road2", "road3"],
  ["road62", "road4", "road2", "road2", "road3"],
  ["road62", "road4", "road2", "road2", "road3"],
...
const questions = [
  {
    question: "You're driving along Katipunan Avenue approaching the Ateneo gate intersection. There's a STOP SIGN posted, and you can see the designated stop line clearly marked on the road. Traffic is light, and you can see no vehicles approaching from either side.",
    options: ["Slow down and proceed if no traffic is visible", "Come to a complete stop at the stop line, then proceed when safe", "Stop briefly and continue immediately"],
    correct: "Come to a complete stop at the stop line, then proceed when safe",
    wrongExplanation: {
      "Slow down and proceed if no traffic is visible": "Wrong! A stop sign requires a complete stop regardless of traffic conditions.",
      "Stop briefly and continue immediately": "Wrong! You must make a complete stop and properly assess safety before proceeding."
    }
  },
...
const trafficSign = {
    sign: require("../../../../../assets/signs/stop_sign.png"),
};
...
export default function DrivingGame() {
  ...
  const trafficSignRowIndex = 12;
  const trafficSignColIndex = .2;
  ...
  const handleAnswer = (answer) => {
    ...
    if (answer === "Slow down and proceed if no traffic is visible") {
      const targetRow = 12;
      ...
      Animated.timing(scrollY, {
    ...
    } else if (answer === "Stop briefly and continue immediately") {
        ...
        }, 1000); // Simulate a 2-second yield time
    } else if(answer === "Come to a complete stop at the stop line, then proceed when safe"){
        const targetRow = 10;
        const rowsToMove = targetRow - currentRow;
        const nextTarget = currentScroll.current + rowsToMove * tileSize;
        ...
        }, 3000); // Simulate a 2-second yield time
    }
  };
  ...
  const handleNext = () => {
    ...
    if (questionIndex < questions.length - 1) {
      ...
      router.push('/driver-game/signs/phase-2/S3P2');
      setQuestionIndex(0);
      setShowQuestion(false);
    }
  };
  ...
  const feedbackMessage = isCorrectAnswer
    ? "Correct! You must come to a complete stop at the designated stop line, check for traffic, then proceed when safe."
