...
console.log('S3P1 Module loaded. API_URL:', API_URL);
...
export default function DrivingGame() {
  ...
  useEffect(() => {
    console.log('useEffect triggered for fetchScenarioData');
    fetchScenarioData();
  ...
  const fetchScenarioData = async () => {
    ...
    try {
      ...
      // Check if response has scenario data (your backend format)
      if (data && data.scenario_choices && data.description) {
        console.log('Processing database scenario data');
        setScenarioData(data);
        
        // Transform database data to match your question format
        ...
        console.log('Database questions loaded successfully:', transformedQuestions);
      } else {
        console.log('Database response format unexpected, using fallback');
        throw new Error('Database response format unexpected');
      }
    } catch (error) {
      console.error('Database error, using fallback questions:', error);
      // Fallback to original questions if database fails
  ...
  function startScrollAnimation() {
    console.log('startScrollAnimation called');
    ...
    console.log('Starting scroll animation from', startOffset, 'to', stopOffset);
    ...
    Animated.timing(scrollY, {
      ...
      console.log('Scroll animation completed, showing question');
  ...
  console.log('Rendering main game. Loading:', loading, 'Questions length:', questions.length, 'Question index:', questionIndex);

  // Safety check - make sure we have valid question data
  if (!questions[questionIndex]) {
    console.log('No valid question data for index:', questionIndex);
    return (
      <View style={{ flex: 1, backgroundColor: "black", justifyContent: 'center', alignItems: 'center' }}>
        <Text style={{ color: 'white', fontSize: 18 }}>Error loading scenario data</Text>
      </View>
    );
  }
