...
import { router } from 'expo-router'; // Changed from useNavigation to router
...
// Backend API configuration - Update this with your actual backend URL
...
export default function PhaseSelectionScreen() {
  ...
  const saveUserProgress = async (selectedPhase) => {
    try {
      setIsLoading(true);
      ...
      const categoryData = await AsyncStorage.getItem('selectedCategory');
      ...
      const response = await fetch(`${API_BASE_URL}/user/progress`, {
        ...
        body: JSON.stringify({
          action: 'phase_selection',
          selectedPhase: selectedPhase,
          categoryId: category?.id || null,
          categoryName: category?.name || null,
          screen: 'phase_selection',
          timestamp: new Date().toISOString(),
        }),
      });
      
      if (response.ok) {
        console.log('Phase progress saved successfully');
      } else {
        console.log('Failed to save phase progress');
      }
      
      // Store locally
      await AsyncStorage.setItem('selectedPhase', selectedPhase);
  ...
  const handleDriverPress = async () => {
    ...
    Animated.sequence([
      ...
      if (success) {
        router.push('/scenarios/s1p1'); // Navigate to first scenario
      }
    });
  };
      ...
      <View style={styles.selectionContainer}>
        <Animated.View style={[{ transform: [{ scale: driverScale }] }]}>
          ...
          >
            <View style={styles.iconContainer}>
              <Image
                source={require('../../assets/icon/steeringwheel.png')} // Fixed path
        ...
        <Animated.View style={[{ transform: [{ scale: pedestrianScale }] }]}>
          ...
          >
            <View style={styles.iconContainer}>
              <Image
                source={require('../../assets/icon/commuter.png')} // Fixed path
                style={styles.optionImage}
                resizeMode="contain"
              />
            </View>
