...
const questions = [
  {
    question: "You're looking for parking in Makati's business district and find a space near your destination. However, you notice a NO PARKING FIRE HYDRANT sign and see a fire hydrant about 3 meters away from the available space.",
    options: ["Park there since you're more than 2 meters from the hydrant", "Look for another parking space that's not near a fire hydrant", "Park there briefly for a quick errand"],
    correct: "Look for another parking space that's not near a fire hydrant",
    wrongExplanation: {
      "Park there since you're more than 2 meters from the hydrant": "Wrong! The law requires no parking within 4 meters of a fire hydrant.",
      "Park there briefly for a quick errand": "Wrong! You cannot tell when an emergency is going to happen. Duration of parking doesn't matter; the restriction applies at all times."
    }
  },
...
const trafficSign = {
    sign: require("../../../../assets/signs/fire_hydrant.png"),
};
...
export default function DrivingGame() {
  ...
  const handleAnswer = (answer) => {
    ...
    if (answer === "Park there since you're more than 2 meters from the hydrant") {
      const targetRow = 4;
    ...
    } else if (answer === "Look for another parking space that's not near a fire hydrant") {
        const targetRow = 9;
        const rowsToMove = targetRow - currentRow;
        const nextTarget = currentScroll.current + rowsToMove * tileSize;
    ...
    } else if(answer === "Park there briefly for a quick errand"){
        const targetRow = 4;
        const rowsToMove = targetRow - currentRow;
        const nextTarget = currentScroll.current + rowsToMove * tileSize;
  ...
  const handleNext = () => {
    ...
    if (questionIndex < questions.length - 1) {
      ...
      router.push('/driver-game/signs/phase-2/S7P2');
      setQuestionIndex(0);
      setShowQuestion(false);
    }
  };
  ...
  const feedbackMessage = isCorrectAnswer
    ? "Correct! You must find parking that doesn't violate the fire hydrant restriction."
