...
const questions = [
  ...
  //   correct: "Option A",
  //   wrongExplanation: {
  //     "Option B": "Explanation for why Option B is wrong.",
  //     "Option C": "Explanation for why Option C is wrong."
  //   }
  // }
];
...
const animateOvertake = async (targetX) => {
  ...
  const handleAnswer = (answer) => {
    ...
    } else if (answer === "Overtake immediately since there's only one solid line") {
    ...
    } else if (answer === "Wait for a safe opportunity, then overtake if the opposite lane is clear") {
        setTimeout(() => {
            ...
            handleFeedback(answer); // Provide feedback once the overtake animation has started
        }, 3000);
      }
  };
  ...
  const handleNext = () => {
    ...
    if (questionIndex < questions.length - 1) {
      ...
      router.push('/driver-game/road-markings/phase-1/S2P1');
      setShowQuestion(false);
      if (scrollAnimationRef.current) {
        scrollAnimationRef.current.stop();
      }
