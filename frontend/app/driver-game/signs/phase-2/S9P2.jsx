...
const questions = [
  {
    question: "You're driving along Recto Avenue near universities and see a LOADING AND UNLOADING ZONE 7AM-9AM, 5PM-7PM sign. It's currently 3:00 PM and you need to pick up documents from a nearby office quickly.",
    options: ["Park there since it's outside the restricted hours", "Find regular parking since you're not loading/unloading passengers", "Use the zone briefly for your errand"],
    correct: "Park there since it's outside the restricted hours",
    wrongExplanation: {
      "Find regular parking since you're not loading/unloading passengers": "Wrong! While considerate, it's not legally required outside restricted hours.",
      "Use the zone briefly for your errand": "Not the best answer! The time restriction is what matters, not the purpose of your stop."
    }
  },
...
const trafficSign = {
    sign: require("../../../../assets/signs/no_loading_and_unloading_time.png"),
};
...
export default function DrivingGame() {
  ...
  const handleAnswer = (answer) => {
    ...
    if (answer === "Park there since it's outside the restricted hours") {
      const targetRow = 4;
    ...
    } else if (answer === "Find regular parking since you're not loading/unloading passengers") {
        const targetRow = 10;
        const rowsToMove = targetRow - currentRow;
        const nextTarget = currentScroll.current + rowsToMove * tileSize;
    ...
    } else if(answer === "Use the zone briefly for your errand"){
        const targetRow = 4;
        const rowsToMove = targetRow - currentRow;
        const nextTarget = currentScroll.current + rowsToMove * tileSize;
  ...
  const handleNext = () => {
    ...
    if (questionIndex < questions.length - 1) {
      ...
      router.push('/driver-game/signs/phase-2/S10P2');
      setQuestionIndex(0);
      setShowQuestion(false);
    }
  };
  ...
  const feedbackMessage = isCorrectAnswer
    ? "Correct! Outside the specified hours, you can use the space for parking."
